...
from apps.planContable.api.serializers import PlanContableSerializer, ListPlanContableSerializer, ListCuentaSerializer
from apps.planContable.models import PlanContable, Cuenta


class PlanContableViewSet(viewsets.GenericViewSet):
    model = PlanContable
    serializer_class = PlanContableSerializer
    list_serializer_class = ListPlanContableSerializer
    ...
    def retrieve(self, request, pk=None):
        ...
        if record:
            record_serializer = PlanContableSerializer(record)
    ...
    @action(methods=['GET'], detail=False, url_path="cuentas", url_name="cuentas")
    def listCuentas(self, request):
        Activo = ListCuentaSerializer(Cuenta.objects.all(), many=True)
